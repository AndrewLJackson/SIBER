...
siber.example <- create.siber.object(demo.siber.data)
...
Various plotting options are collated into lists and then passed to the high-level SIBER plotting function `plot.siber.object` which is a wrapper function for easy plotting. We will access the more specific plotting functions directly a little later on to create more customised graphics.
...
After calling `siber.MVN()` you will see output in the command window indicating that the jags models are being fitted, one block of output for each group in your dataset. A subset of these blocks are shown below.
