...
pack_boundaries <- packs |> purrr::map(\(x) siberKapow(x, isoNames = c("c13","n15"), 
...
ellipse.areas <- pack_boundaries |> map(\(x) extractProportions(x))
...
df_proportions <- ellipse_proportions |> 
  map_df(\(x) x |> 
           map_df(\(y) data.frame(y),  .id = "individual"), 
         .id = "pack")

# rename the ugly variable manually
df_proportions <- rename(df_proportions, Proportion = "y")
