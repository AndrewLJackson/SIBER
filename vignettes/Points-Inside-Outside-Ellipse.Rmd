...
inside_test <- ellipseInOut(Z_test, p = p, method = "qChi")
...
testEllipse <- function(n, p) {
  ...
  # test if inside
  inside_samp <- ellipseInOut(Z_samp, p = p, method = "qChi")
...
do_these_n <- c(5:50, 100, 320, 1000)
...
**Figure 3.** The effect of sample size on the proportion of samples falling inside the 95% prediction ellipse (black points - jittered) with the mean shown in red. At lower samples sizes, the possible combinations are more granular, and cause the step-wise shifts: e.g. at $n$=10 there are either 9/10 or 10/10 points inside the ellipse.
