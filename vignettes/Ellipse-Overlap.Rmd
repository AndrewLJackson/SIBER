...
vignette: >
  ...
  %\VignetteDepends{viridis, spatstat}
---
...
In order now to calculate the overlap between two (or more) ellipses, we need to know the coordinates of each ellipse. This is done by manually calling `addEllipse(..., do.plot = FALSE)`. See the associated help file and the vignette [Customising-Plots-Manually](Customising-Plots-Manually.html) for more information on optional inputs to addEllipse for different types of ellipse. Also, bear in mind that the option `n` controls how many points are used to draw the ellipse, and hence low `n` means clunky, edgier ellipses, compared with rounder, smoother ellipses for higher `n`. A higher `n` is more suitable when ellipses are more eccentric as their curvature is greater at the tips. 
