...
vector_data <- allCentroidVectors(centroids, do.plot = FALSE)
...
# species
paired.names <- paste0(paste0(siber_rodents$all.communities[2], ".",
                           siber_rodents$group.names[[1]]), "|",
                    paste0(siber_rodents$all.communities[1], ".",
                           siber_rodents$group.names[[1]]))
...
hist.by.groups <- vector_data %>% group_by(comparison) %>%
                             do(my.hist(.))
...
median_vectors <- dplyr::summarise(group_by(vector_data, comparison),
...
cart_positions <- with(vector_data, data.frame(x = distances * cos(angle),
                             y = distances * sin(angle),
                             comparison = comparison ))

# rename as above
bb <- unlist(strsplit(as.character(vector_data$comparison), "[.]"))
...
ggplot(cart_positions, aes(x,y) ) + 
  ...
  facet_wrap( ~ comparison2, scales = "fixed") + 
  theme_classic() +
