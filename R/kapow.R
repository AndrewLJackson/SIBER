...
kapow <- function(cd = 7, ng = 25, n = 50, sc = 15, do.plot = TRUE) {
  
  # 3 or 7 seem pretty good with the other defaults above.
  set.seed(cd)
  
  Y <- generateSiberCommunity(n.groups = ng, n.obs = n, wishSigmaScale = sc)
  
  Y <- Y %>% dplyr::mutate(group = factor(group))
  ...
  p <- ggplot(ellY$ell.coords, 
    ...
    annotate("text", x = 0, y = 0, label = "KAPOW!",
              size = 30, angle = 10, family = "Komika Display", 
             color = "#F3F315") + 
    theme_classic() + 
