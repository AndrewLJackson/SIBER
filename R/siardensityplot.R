...
#' Defaults to greyscale \code{gray((9:1)/10)}.
...
#' Useful for example when plotting dietary proportions which must lie in 
#' the interval \code{0 <= Y <= 1}. Defaults to -Inf
  ...
  for (j in 1:n) {
    ...
    if (prn == TRUE) {
      ...
      cat(paste("\t", "Mode",
                ...
                "Median",
                format(median(dat[, j]), digits = 3, scientific = F), "\n")
          )
    }
